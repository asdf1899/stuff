...
<script>
    ...
    export default {
      ...
      mounted(){
        ...
        axios.all([
          axios.get('https://anasaraid.me/stuff-data/articles.json'),
          axios.get(markdownUrl)
          ])
          .then(
          axios.spread(
            function (articlesData, markdownData) {
              that.currentArticle = articlesData.data;
              that.fileContent = markdownData.data;
            }
          ))
        */
        let markdownUrl = 'https://anasaraid.me/stuff-data/markdowns/'+this.id+'.md';
